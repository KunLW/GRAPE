...
2. **`experiments_improved/`** — the modern driver. `run_experiment.py` is sealed: it reaches every system-specific fact (Hamiltonians, noise, bounds, targets, plot labels) through a system-definition object resolved from the YAML `system.type` via the registry in `systems/__init__.py`. **Invariant: the driver and `quantum_control` core must never import concrete system modules.** Adding a new physical system = one subclass of `SystemDefinitionBase` (`systems/common.py`) + one registry line, zero driver edits — follow `systems/README.md`; `systems/spin_boson.py` is the reference subclass.
