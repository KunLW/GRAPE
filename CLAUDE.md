...
1. **`quantum_control/`** — the system-agnostic GRAPE engine (installable package). Data flow:
   ...
   - *Decoherence (short-correlation)*: `LindbladExpansionEvolution` + `LindbladCorrectedStateFidelity` compute an additive first-order correction; `SumProblem` sums the two into the optimizer objective.
   `StateAverageProblem` averages any of these over weighted state pairs (with optional worker processes); problem assembly (context, `ControlProblem`, state averaging, parameterization wrapper, penalties) lives in `quantum_control/problems/`. Reporting-side metrics live in `quantum_control/evaluation/`: `closed_gate_fidelity(system, pulse, state_pairs)` and `noisy_gate_fidelity(system, pulse, state_pairs, collapse_operators=())` — the latter equals the optimizer's raw objective (expansion + Lindblad correction) — plus `faithful_gate_fidelity(open_system, pulse, state_pairs, hermite_points=5)`, the exact check: full Lindblad density-matrix propagation with Gauss–Hermite averaging over the fluctuations (cost `hermite_points ** n_fluctuation_terms` — evaluation only, exposed via `evaluate --faithful`). `quantum_control` contains zero concrete physics (grep-checkable: no `spin_boson`, MS-gate, or test-state helpers anywhere in it).
...
- **Noise scaling**: a `FluctuationTerm` scales linearly (`matrix = sigma * A`); a `DecoherenceChannel` scales through the jump operator (`matrix = sqrt(gamma) * A`). Terms are declared unscaled; `SystemDefinitionBase.build_systems` applies the YAML `enabled`/`any_sigma_positive`/`any_rate_positive` gating when assembling the `OpenSystem` (fluctuation terms all-or-nothing — control-kind terms align with control channels positionally — decoherence channels filtered per channel).
