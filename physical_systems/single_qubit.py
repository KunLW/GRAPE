...
(``experiments/single_qubit/configs/example.yaml``). Drop a module like this
...
Physical model
--------------
One qubit in the frame rotating at the drive frequency (``hbar = 1``):

    H(t) = 0.5 * delta * sigma_z + 0.5 * omega(t) * sigma_x

``delta`` is a constant drive detuning (the drift) and ``omega(t)`` the
piecewise-constant Rabi amplitude of a resonant drive (the single control
channel). The optimization target is the X gate (a pi pulse), measured as the
gate fidelity averaged over the four tomography input states ``|0>``, ``|1>``,
``|+>``, ``|+i>``.

Only the five required hooks of ``SystemDefinitionBase`` are implemented
(``name``, ``default_params``, ``build_closed_system``, ``control_bounds``,
``state_pairs``) plus two tiny optional ones (``target_gate`` for reporting,
``control_channels`` for kHz-labelled plots). Everything else — the
``OpenSystem`` assembly, the flat initial pulse at the bounds midpoint, the
bounded parameterization — comes from the base class defaults, and with no
noise hooks the system is closed-only (``system.noise`` stays disabled).
