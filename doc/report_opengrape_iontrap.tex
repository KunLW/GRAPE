...
\newpage
\section{Ion Trap and laser interaction}
...
\end{equation}
where $S_\phi = \vb b_k \cdot \vb*\sigma_\phi$, $\vb b_k$ is the mode vector of the mode $k$. For com mode, $\vb b_k = (1,1)$, for stretch mode, $\vb b_k = (1,-1)$.

For the stretch mode, the two ions will move in opposite direction, so the effective Hamiltonian can be written as:
\begin{equation}
    H_{\textit{stretch}} = \frac{\Omega}{2} \eta \cdot (\sigma_{\phi_s}^{(1)} - \sigma_{\phi_s}^{(2)}) \otimes (a_k e^{i(\delta t - \phi_m)} + a_k^\dagger e^{-i(\delta t - \phi_m)})
\end{equation}
...
\begin{align*}
    H = \alpha_1 a^\dagger a + \alpha_2 S_{\phi_s} \otimes (a_k + a_k^\dagger)
\end{align*}
...
\begin{equation}
    \dot \rho = -i[H(t; \vb* \xi, \vb* \chi), \rho] + \sum_j \kappa (L_j \rho L_j^\dagger - \frac{1}{2} L_j^\dagger L_j \rho - \frac{1}{2} \rho L_j^\dagger L_j)
\end{equation}
...
\begin{equation}
    W_j = \exp\left\{-i\tau \left(H_0(t_j) + \sum_{i} c_i(t_j) H_i \right)\right\}
\end{equation}
...
\begin{equation}
    ...
    \label{eq:fluc_expansion}
\end{equation}
where $W_0 = \ketbra{\psi_0}$, $W_{-1} = W_{1}^\dagger$

To calculate the derivative of the open gate fidelity, we need first define following six states group, for $k = 1, 2,\cdots, N$:
\begin{itemize}
...
\end{itemize}
Then the \eqref{eq:fluc_expansion} can be decomposed by thinking of putting two indistinguishable $V$ to 4 different positions
\begin{itemize}
...
The derivative of the propagator $W$ and $V$ can be calculated by the Frechet derivative of the matrix exponential. With the condition $\tau\leq \norm{H}^{-1}$, we can get the expansion of Dyson and remain the first order term. 
\begin{align}
    \pdv{W_k}{c_i(j)}
    &=
    -i\tau H_i W_k
